...
namespace cuda_filter
{
    ...
    {
        if (str == "webcam") return InputSource::WEBCAM;
        if (str == "image") return InputSource::IMAGE;
        if (str == "video") return InputSource::VIDEO;
        if (str == "synthetic") return InputSource::SYNTHETIC;
    ...
    {
        if (str == "checkerboard") return SyntheticPattern::CHECKERBOARD;
        if (str == "gradient") return SyntheticPattern::GRADIENT;
        if (str == "noise") return SyntheticPattern::NOISE;
    ...
    FilterOptions InputArgsParser::parseArgs()
    {
        ...
        // Parse new pipeline and existing parameters
        filterOptions.pipelineString = result["pipeline"].as<std::string>();
    ...
    void InputArgsParser::setupOptions(cxxopts::Options &options)
    {
        options.add_options()
            ...
            ("p,path", "Path to input file", cxxopts::value<std::string>()->default_value("test_image.jpg"))
            ("s,synthetic", "Pattern: 'checkerboard', 'gradient', 'noise'", cxxopts::value<std::string>()->default_value("checkerboard"))
            ("d,device", "Camera device ID", cxxopts::value<int>()->default_value("0"))
            ("pipeline", "Pipeline string: e.g. 'blur,hdr,wipe'", cxxopts::value<std::string>()->default_value("blur,wipe"))
            ("f,filter", "Legacy filter type", cxxopts::value<std::string>()->default_value("blur"))
            ("k,kernel-size", "Kernel size", cxxopts::value<int>()->default_value("3"))
            ("sigma", "Sigma value", cxxopts::value<float>()->default_value("1.0"))
            ("intensity", "Filter intensity", cxxopts::value<float>()->default_value("1.0"))
            ("preview", "Show preview")
            ("h,help", "Print usage")
            ("v,version", "Print version")
            ("exposure", "HDR Exposure", cxxopts::value<float>()->default_value("1.0"))
            ("gamma", "HDR Gamma", cxxopts::value<float>()->default_value("2.2"))
            ("saturation", "HDR Saturation", cxxopts::value<float>()->default_value("1.0"));
    }
