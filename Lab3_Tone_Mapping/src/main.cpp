...
int main(int argc, char **argv)
{
    ...
    cuda_filter::HDRProcessor hdrProc;
    ...
    while (true)
    {
        ...
        if (filterType == cuda_filter::FilterType::HDR_TONEMAPPING) {
          cuda_filter::applyHDRToneMappingCPU(frame, filteredCPU, options.exposure, options.gamma, options.saturation);
        } else {
        ...
        if (filterType == cuda_filter::FilterType::HDR_TONEMAPPING) {
          cuda_filter::applyHDRToneMappingGPU(hdrProc, frame, filteredGPU, 
                                                options.exposure, options.gamma, options.saturation);
        } else {
        ...
        cv::imwrite("/content/result.jpg", combined);
        PLOG_INFO << "result is saved in /content/result.jpg";
        break;
    }
